...
<!--<script src="<c:url value="${context}js/ckeditor/ckeditor.js" />" ></script>-->
...
<div class="card mb-3 p-2">
    <div class="container ">
        <div class="row d-flex align-items-center">
            <div class="col-md-12">
                <div class="mb-3">
                    <label for="exampleTextField" class="form-label">
                        Subject
                    </label>
                    <input data-group-id="${myGroup.id}" type="text"
                           class="form-control" id="subject"  placeholder="Enter text" 
                           />
                </div>
                <div  id="container">
                ...
                <button class="btn btn-warning" onclick="sendBulkMail()" />
        ...
        <div class="container ">
            <div class="row d-flex align-items-center">
                <div class="col-md-9">
                    <div class="mb-3">
                        <label for="exampleTextField" class="form-label">
                            <spring:message code="view.pages.group-user.group-name" />
                        </label>
    ...
    <script>
        ...
        const sendBulkMail = () => {
            const mailUserGroup = Array.from(document.querySelectorAll('.group-member')).filter(input => input.checked).map(item => item.getAttribute("data-email")).filter(value => value != null);
            const editorData = editor.getData();
            const subject = document.querySelector("#subject").value;
            console.log(subject);
            console.log(editorData);
            console.log(mailUserGroup);
            const data = {
                mailTos: mailUserGroup,
                subject: subject,
                content: editorData
            };
            fetch('<c:url value="/admin/community/send-bulk" />', {
                method: "POST",
                headers: {
                    'Content-Type': 'application/json' // Tell the server we're sending JSON data
                },
